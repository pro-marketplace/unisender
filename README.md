...
## Пошаговая настройка Unisender Go
...
### Шаг 5: Получение API-ключа и тестовое письмо
...
### Шаг 6: Переход на платный тариф (опционально)
...
**ОБЯЗАТЕЛЬНО:**
1. Добавь тестовую кнопку для проверки настройки
2. Интегрируй отправку писем в нужные места приложения

Используй компоненты из `/frontend`:
- `useUnisender.ts` — хук для отправки писем
- `UnisenderTestButton.tsx` — кнопка проверки (исчезает после успешного теста)
